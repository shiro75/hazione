...
import type { Supplier, ExpenseType, ExpenseStatus } from '@/types';
...
function DepensesSection({ _isMobile }: { _isMobile?: boolean }) {
  const { colors } = useTheme();
  const { t } = useI18n();
  ...
  const openCreate = useCallback(() => {
    setEditingId(null);
    setForm({
      expenseType: 'autre', description: '', amount: 0, vatAmount: 0, vatRate: 20,
      date: new Date().toISOString(), supplierName: '', reference: '',
      paymentMethod: 'cash', status: 'pending', notes: '',
    });
    setFormError('');
    setFormVisible(true);
  }, []);
  ...
  const openEdit = useCallback((id: string) => {
    ...
    setEditingId(id);
    setForm({
      expenseType: e.expenseType, description: e.description, amount: e.amount,
      vatAmount: e.vatAmount, vatRate: e.vatRate, date: e.date,
      supplierName: e.supplierName, reference: e.reference,
      paymentMethod: e.paymentMethod, status: e.status, notes: e.notes,
    });
    setFormError('');
    setFormVisible(true);
  }, [activeExpenses]);
  ...
  return (
    <>
      <View style={[styles.searchRow, { marginBottom: 8 }]}>
        <View style={[styles.kpiCard, { backgroundColor: colors.card, borderColor: colors.cardBorder, flex: 1, borderWidth: 1, borderRadius: 10, padding: 12 }]}>
          <Text style={{ fontSize: 12, color: colors.textSecondary }}>Total dépenses</Text>
          <Text style={{ fontSize: 18, fontWeight: '700' as const, color: colors.danger }}>{formatCurrency(totalExpenses, cur)}</Text>
        </View>
        <View style={[styles.kpiCard, { backgroundColor: colors.card, borderColor: colors.cardBorder, flex: 1, borderWidth: 1, borderRadius: 10, padding: 12 }]}>
          <Text style={{ fontSize: 12, color: colors.textSecondary }}>Nombre</Text>
          <Text style={{ fontSize: 18, fontWeight: '700' as const, color: colors.text }}>{activeExpenses.length}</Text>
        </View>
      </View>
      ...
      <View style={styles.searchRow}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            const data = activeExpenses.map(e => ({ ...e } as unknown as Record<string, unknown>));
            void exportToCSV(data, cols, `depenses_${new Date().toISOString().slice(0, 10)}.csv`);
          }}
        >
          <Download size={16} color={colors.text} />
        </TouchableOpacity>
      ...
      {filtered.length === 0 ? (
        <View style={styles.emptyState}>
          ...
          <Text style={[styles.emptyTitle, { color: colors.textSecondary }]}>
            {search ? 'Aucun résultat' : t('expenses.noExpenses')}
          </Text>
          <Text style={[styles.emptySubtitle, { color: colors.textTertiary }]}>
            {search ? 'Essayez un autre terme' : t('expenses.addFirst')}
          </Text>
        </View>
      ) : (
        <View style={[styles.tableCard, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
          {filtered.map((expense, idx) => (
            <TouchableOpacity
              key={expense.id}
              style={[styles.listRow, idx < filtered.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.borderLight }]}
              onPress={() => openEdit(expense.id)}
              activeOpacity={0.7}
            >
              <View style={styles.listRowMain}>
                <View style={styles.listRowInfo}>
                  ...
                  <Text style={[styles.listRowSub, { color: colors.textSecondary }]}>
                    {getTypeLabel(expense.expenseType)} {expense.supplierName ? `— ${expense.supplierName}` : ''}
                  </Text>
                  <Text style={{ fontSize: 11, color: colors.textTertiary, marginTop: 2 }}>
      ...
      <FormModal
        visible={formVisible}
        onClose={() => setFormVisible(false)}
        title={editingId ? t('expenses.edit') : t('expenses.new')}
        onSubmit={handleSubmit}
        headerActions={editingId ? (
      ...
      >
        <View style={{ marginBottom: 16 }}>
          <Text style={{ fontSize: 13, fontWeight: '500' as const, color: colors.textSecondary, marginBottom: 6 }}>{t('expenses.type')}</Text>
          <TouchableOpacity
            style={[styles.formPickerBtn, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder }]}
        ...
        <View style={{ marginBottom: 16 }}>
          <Text style={{ fontSize: 13, fontWeight: '500' as const, color: colors.textSecondary, marginBottom: 6 }}>{t('expenses.paymentMethod')}</Text>
          <TouchableOpacity
            style={[styles.formPickerBtn, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder }]}
            onPress={() => setShowPaymentPicker(p => !p)}
          >
        ...
        <View style={{ marginBottom: 16 }}>
          <Text style={{ fontSize: 13, fontWeight: '500' as const, color: colors.textSecondary, marginBottom: 6 }}>{t('expenses.status')}</Text>
          <TouchableOpacity
            style={[styles.formPickerBtn, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder }]}
...
const styles = StyleSheet.create({
  ...
  kpiCard: {},
  formPickerBtn: { flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'space-between' as const, borderWidth: 1, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 12 },
  formPickerDropdown: { borderWidth: 1, borderRadius: 8, marginTop: 4, overflow: 'hidden' as const },
  formPickerOption: { flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'space-between' as const, paddingVertical: 10, paddingHorizontal: 14, borderBottomWidth: 1 },
  formCol: { flex: 1 },
